use crate::crypto::boring::error::{br, br_zero_is_success, BoringResult};
...
pub trait QuicSslContext {
    ...
    fn set_private_key<T: HasPrivate>(&mut self, key: &PKeyRef<T>) -> BoringResult;
    fn check_private_key(&self) -> BoringResult;
    fn set_certificate(&mut self, cert: &X509Ref) -> BoringResult;
    fn cert_store_mut(&mut self) -> &mut X509StoreBuilderRef;
    fn add_extra_chain_cert(&mut self, cert: X509) -> BoringResult;

    /// Utility method that calls set_certificate for the leaf (first cert in the list)
    /// and [add_extra_cert_chain] for all subsequent certs.
    fn set_cert_chain(&mut self, cert_chain: Vec<X509>) -> BoringResult;
...
impl QuicSslContext for SslContext {
    ...
    fn set_private_key<T: HasPrivate>(&mut self, key: &PKeyRef<T>) -> BoringResult {
        unsafe { br(bffi::SSL_CTX_use_PrivateKey(self.as_ptr(), key.as_ptr())) }
    }
    ...
    fn set_certificate(&mut self, cert: &X509Ref) -> BoringResult {
        unsafe { br(bffi::SSL_CTX_use_certificate(self.as_ptr(), cert.as_ptr())) }
    }
